...
# !encoding:utf-8
##################
#这个程序是将bom清单里2个以上(不含2个)连续的位号合并成CE1~CE3的形式，适合25所标准化要求；
...
# 示例输入
# input_list = "1,2,3,5,6,8,10,11,12,14"
# nums = list(map(int, input_list.split(',')))
# ranges = convert_to_ranges(nums)
# output = format_ranges(ranges)
# print(output)
...
def import_excel(file1):
    ...
    File_Name=file1
    New_File_Name =os.path.splitext(file1)[0] + '_Convert' + os.path.splitext(file1)[1]
    File_Log_Name = os.path.splitext(file1)[0] + '_Convert' + '.log'  #记录的日志文件
...
# df.reset_index()
    ...
    while i<max_rows :
        ...
        part_count = df.iloc[i,REFERENCE_COLUMN]
        ...
        reference_num_list=reference_num_list.split(',')
        letters=extract_letters_from_list(reference_num_list)
        letter= letters[0]
        print(f"位号的字母部分内容:{letter}")
        file_log.write(f"位号的字母部分内容:\n{letter}"+"\n")
        reference_num_list_num=extract_numbers_from_list(reference_num_list)
        print(f"位号的数字部分内容:{reference_num_list_num}")
        file_log.write(f"位号的数字部分内容:\n{reference_num_list_num}"+"\n")
        # reference_num_list=reference_num_list.split(',')
        # reference_num_list=sorted(reference_num_list.split(','),key=compare_func)  # 提取每个元素后面的数值部分并转换成int类型
        reference_num_list_num=convert_to_range_list(reference_num_list_num)
        print(f"变换为起止数字后的内容:{reference_num_list_num}")
        file_log.write(f"变换为起止数字后的内容:\n{reference_num_list_num}"+"\n")
        reference_num_list_num=format_ranges(reference_num_list_num,letter)
        # print(f"转换为列表数字后的内容:{reference_num_list_num}")
        # file_log.write(f"转换为列表数字后的内容:\n{reference_num_list_num}"+"\n")
        # reference_num_list=add_prefix_to_ranges_and_single_numbers(reference_num_list_num,letter)
        print(f"转换完的内容:{reference_num_list_num}")
        file_log.write(f"转换完的内容:\n{reference_num_list_num}"+"\n")
        # reference_num_list_num=','.join(reference_num_list_num)  #将列表转换成以逗号分隔的字符串
        df.iloc[i,REFERENCE_COLUMN]=reference_num_list_num
